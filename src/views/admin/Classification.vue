    ...
    <el-row :gutter="25">
      <el-col :span="6">
        <el-input
          ...
          @clear="getClassifList"
        >
          <!-- 内容清除后再查询一遍 -->
          <el-button
            slot="append"
            icon="el-icon-search"
            @click="getNewClassifList"
          ></el-button>
        </el-input>
      </el-col>
      <!-- 搜索按钮 -->
...
export default {
  ...
  methods: {
    ...
    //获取分类列表
    async getClassifList() {
      // 调用post请求
      const { data: res } = await this.$https.get("allclassification", {
        params: this.queryInfo,
      });
      this.classifList = res.data; // 将返回数据赋值，数据封装
    ...
    async deleteUser(id) {
      ...
      const { data: res } = await this.$https.delete("deleteClassfi?id=" + id);
      if (res !== "success") {
    ...
    async handleBatchDelete() {
      this.$https
        .put("batchdelete", this.multipleSelection)
        .then((res) => {
          console.log(res); // 添加这一行来检查响应内容
          if (res.data === "success") {
            // 确保使用res.data来获取响应体
            this.$message.success("批量删除成功！");
            this.getClassifList(); // 重新获取分类列表
          } else if (res === "error2") {
            console.log("重复");
    ...
    async showEditDialog(id) {
      ...
      const { data: res } = await this.$https.get("classifiupdate?id=" + id);
      this.editForm = res;
